...
if __name__ == '__main__':
    ...
    model=Sequential([
        Embedding(max_words + 1, output_dim=vector_size, input_length=input_size),
        Conv1D(filters=state_size, kernel_size=16, padding='same', activation='relu'),
        MaxPooling1D(),
        Conv1D(filters=round(state_size / 4), kernel_size=8, padding='same', activation='relu'),
        MaxPooling1D(),
        Flatten(),
    ...
    # print_layer_config(model.layers)
    model.compile(loss=losses.binary_crossentropy, optimizer='adam', metrics=['accuracy'])
    # Train the model
    model.fit(X_train, Y, epochs=epochs, batch_size=batch_size, validation_data=(X_eval, Yv), callbacks=[stats])
    ...
    accuracy = model.evaluate(X_test, Yt, verbose=1, callbacks=[stats])
    print("Accuracy: {}%".format(round(accuracy[1]*100, 2)))
